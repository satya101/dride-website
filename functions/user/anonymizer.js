...
anonymizer = {
     ...
     */
	startFS: function(needToAnonymise, userRef, uid) {
		var promiseArr = [];
		return new Promise((resolve, reject) => {
			if (!needToAnonymise) {
					...
					.then(function(userRecord) {
						...
						return admin
							...
							.then(function(userRecord) {
								...
								var ref = db.collection('users').doc(userRef);
								var newDetailsObj = {
									photoURL: user.providerData[0].photoURL,
									pic: user.providerData[0].photoURL,
									name: user.providerData[0].displayName
								};
								promiseArr.push(ref.update(newDetailsObj));
								promiseArr.push(
									anonymizer.updateCommentsUser(uid, user.providerData[0].displayName, user.providerData[0].photoURL)
								);
							})
				...
				promiseArr.push(ref.update(newDetailsObj));
				anonymizer.updateCommentsUser(uid, newDetailsObj.name, newDetailsObj.photoURL);
				promiseArr.push(
	...
	getRandomName: function() {
		var nickNames = [
			...
			'The Iceman',
			'The Black Knight',
			'Worlds Fastest Austrian',
			'The Whip',
			'GO',
			'Wild Bill',
			'Worlds Fastest Mexican',
			"Flamin' Frank",
			'Jet Car Bob',
			'Parachute Potter',
			'Dirty Eddie',
			...
			'Sandman',
			'The Bear',
			'Fat Belly. The Shadow',
			'King of Garden Grove',
			'PF Flyer',
			'Pattyfaster',
			'The Action Man',
			'The Beachcomer',
			...
			'Eddie Munster',
			'Rapid Ronnie',
			"World's Fastest Hippie"
		];
		return nickNames[parseInt(Math.random() * nickNames.length)];
	},
	updateForumUser: function(uid, name, pic) {
		//inner queries are not supported yet, let's take car of this when they're live..
	},
	updateCommentsUser: function(uid, name, pic) {
		var db = admin.firestore();
