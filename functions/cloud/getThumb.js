       ...
       */
			...
			bucket.file(filePath).download(
				...
				function(err) {
					...
					video.prepareVideoToCloud().then(
						res => {
							console.log('uploading..');
							video
								.uploadToBucket(path + uid + '_' + timestamp + '.jpg', 'thumbs/' + uid + '/' + timestamp + '.jpg')
								.then(
									done => {
							...
							promises.push(
								video
									.uploadToBucket(path + uid + '_' + timestamp + '.mp4', 'clips/' + uid + '/' + timestamp + '.mp4')
									.then(
										done => {
							...
							Promise.all(promises).then(values => {
								...
								fs.unlinkSync(path + uid + '__' + timestamp + '.mp4');
								fs.unlinkSync(path + uid + '_' + timestamp + '.mp4');
								fs.unlinkSync(path + uid + '_resized_' + timestamp + '.mp4');
								fs.unlinkSync(path + uid + '_' + timestamp + '/' + uid + '_' + timestamp + '_1.jpg');
